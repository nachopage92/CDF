...
!-------------------------------------------------
	...
	character(len=8)::data_folder='./datos/'
...
!-------------------------------------------------
	...
	do k=1,4
		...
		y_lf(k)%kk(2,:) = y_lf(k)%kk(1,:) + (/ (	 ( exp(-dt)*sin(pi*x(k)%kk_(i)) - y_lf(k)%kk(1,i) ) / dt , i=1,nx+1 	) /)
		y_nm(k)%kk(1,:) = (/ (sin(pi*x(k)%kk_(i)),i=1,nx+1) /)
		dy_nm(k)%kk(1,:) = (/ ( 	(exp(-dt)*sin(pi*x(k)%kk_(i)) - y_nm(k)%kk(1,i))/dt , i=1 , nx+1	) /)
