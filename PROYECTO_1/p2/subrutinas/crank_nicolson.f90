...
!---------------------------------------------
	...
	!variables locales
	integer							:: i,j
	real(kind=np)					:: suma,determinante,dt
	real(kind=np),dimension(2)		:: yn0,yn1,f_0
	real(kind=np),dimension(2,2)	:: Matrix_A,Matrix_B
!---------------------------------------------
	dt = t(2) - t(1)
	Matrix_A(1,:) = (/ beta*dt*0.5_8 - alfa*dt**2._8*0.25_8 ,  dt*0.5_8 /)
	Matrix_A(2,:) = (/ -alfa*dt*0.5_8 , 1._8-beta*dt*0.5_8-alfa*0.25_8*dt**2._8 /)
	Matrix_A=Matrix_A/(beta*dt*0.5_8 + alfa*dt**2._8*0.25_8)
	Matrix_B(1,:) = (/ beta*dt*0.5_8 , dt*0.5_8 /)
	Matrix_B(2,:) = (/ -alfa*dt*0.5_8 , 1._8 /)
	Matrix_B=Matrix_B/(beta*dt*0.5_8 + alfa*dt**2._8*0.25_8)
